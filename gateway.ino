...
const long OpeningTime = 6000; // 1 minuto em milissegundos
...
void loop() {
  ...
  if (packetSize) {
    ...
    long nodeSleepTime = OpeningTime - totalTime;
  ...
  if (totalTime >= OpeningTime) {
    Serial.println("Gateway into sleep mode...");
    Serial.println("\n");
    delay(OpeningTime - totalTime);
    totalTime = 0;
  }
}
...
//   // Verifica se o tempo de duração do handShake é maior que o tempo total de envio de todos os nós
//   if (totalTime <= OpeningTime) {
//     Serial.println("Gateway into sleep mode...");
//     Serial.println("\n");
//     delay(OpeningTime - totalTime);
